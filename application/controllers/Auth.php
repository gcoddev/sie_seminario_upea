...
class Auth extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->helper(['url', 'language', 'captcha', 'funciones_helper']);
		$this->load->Model(['Modelo_administrador', 'Modelo_estudiante']);
	...
	public function index()
	{
		...
		if ($this->ion_auth->in_group('doctor')) {
			...
			redirect(Hasher::make(24), 'refresh');
			//redirect(Hasher::make(24), 'refresh');
		} else {
			if (!$this->ion_auth->logged_in()) {
				// redirect them to the login page
				redirect(Hasher::make(1), 'refresh');
			} else if (!$this->ion_auth->is_admin()) // remove this elseif if you want to enable this for non-admins
			{
				redirect(Hasher::make(2), 'refresh');
			} else {
				$this->data['title'] = $this->lang->line('index_heading');
				$this->output->set_title('SIE|ADMIN');
				...
				//$this->data['users'] = retorna_notifica();
	...
	{
		$not = $this->Modelo_administrador->retorna_notifica();
		echo json_encode($not);
	}
	function estadisticas()
	{
		if ($this->input->post('action') == 'estas') {
			$car = $this->Modelo_administrador->carreras('base_upea.carrera');
	...
	public function login()
	{
		...
		$this->Modelo_administrador->db_insert('visitas', array('ip_visita' => $this->getRealIP(), 'datos' => $_SERVER['HTTP_USER_AGENT']));
		// validate form input
		...
		if ($this->form_validation->run() === TRUE) {
			...
			// el usuario no está iniciando sesión, muestre la página de inicio de sesión
			//configurar el mensaje de error de datos flash si hay uno
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
	...
	public function logout()
	{
		...
		// redirect them to the login page
		redirect(Hasher::make(25), 'refresh');
	}
