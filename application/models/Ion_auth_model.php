...
class Ion_auth_model extends CI_Model
{
	...
	{
		$valor = $this->db->query("SELECT username FROM `users` WHERE username='$identity' ")->row();
		if (!$valor) {
			$gestion = date('Y');
			//estudiante
			$valor1 = $this->db->query("SELECT mae.ci,mae.nombre,mae.paterno,mae.materno,mae.id_estudiante,mae.fecha_nac,mae.carrera_id FROM base_upea.vista_mae_matriculados as mae WHERE mae.ci='$identity' AND mae.gestion='$gestion' ")->row();
			if ($valor1) {
				$usuario1 = $valor1->ci;

				$password1 = date("d-m-Y", strtotime($valor1->fecha_nac));
				$id_usuario = $this->cargar_nuevo_estudiante_usuario_pass_db($usuario1, $password1, $valor1->nombre, $valor1->paterno, $valor1->id_estudiante, $valor1->ci, $valor1->carrera_id);
				$id_rol = 4; //id estudiante que es el 4
				$this->dp_auth_users_groups($id_usuario, $id_rol);

				$this->login($usuario1, $password1, $remember = FALSE);
			} else { //

				$valor2 = $this->db->query("SELECT doc.password, doc.id_persona,doc.username,doc.last_name,doc.first_name,doc.id FROM base_upea.users as doc WHERE doc.username='$identity'  GROUP BY doc.id_persona ")->row();
				/* 		var_dump($valor2);
				die; */
				if ($valor2) {
					$usuario2 = $valor2->username;
					$password2 = $valor2->password;
					/* var_dump($password2);
					die; */
					$id_usuario = $this->cargar_nuevo_docente_usuario_pass_db($usuario2, $password2, $valor2->first_name, $valor2->last_name, $valor2->id_persona, $valor2->id);
					$id_rol = 1; //id adminl 4
					$this->dp_auth_users_groups($id_usuario, $id_rol);

					$this->login($usuario2, $password2, $remember = FALSE);
				} else {
	...
	////filyboy
	public function guardar_nuevo_usuario($first_name, $last_name, $company, $phone, $email, $password, $ci)
	{
	...
	public function dp_auth_users_groups($id_usuario, $id_rol)
	{
		$data = array(
			'user_id' => $id_usuario,
			'group_id' => $id_rol
	...
	public function cargar_nuevo_docente_usuario_pass_db($usuario, $password, $nombre, $paterno, $id_persona, $ci)
	{
		$ip_address = $this->input->ip_address();
		$salt       = $this->store_salt ? $this->salt() : FALSE;
		//$password   = $this->hash_password($password, $salt);
	...
	public function cargar_nuevo_doctor_usuario_pass_db($usuario, $password, $nombre, $paterno, $id_persona, $ci)
	{
		$ip_address = $this->input->ip_address();
		$salt       = $this->store_salt ? $this->salt() : FALSE;
		$password   = $this->hash_password($password, $salt);
	...
	public function cargar_nuevo_estudiante_usuario_pass_db($usuario, $password, $nombre, $paterno, $id_estudiante, $ci, $carrera_id)
	{
		$ip_address = $this->input->ip_address();
		$salt       = $this->store_salt ? $this->salt() : FALSE;
		$password   = $this->hash_password($password, $salt);
	...
	public function listar_dp_auth_users_groups($id)
	{
		return $this->db->query("SELECT * FROM users_groups WHERE id='$id' ")->result();
	}
	public function eliminar_tbl_dp_auth_users_groups($id_user_group)
	{
		$this->db->where("id", $id_user_group);
		$this->db->delete("users_groups");
	}
	public function eliminar_tbl_usuario($id)
	{
		$this->db->where("id", $id);
		$this->db->delete("users");
	}
	public function guardar_usu_editar($id_usuario, $first_name, $last_name, $company, $phone, $ci)
	{
		$data = array(
			'first_name' => $first_name,
			'last_name' => $last_name,
	...
	public function resetear_usu_pass($id_usuario, $usuario_n)
	{
		$ip_address = $this->input->ip_address();
		$salt       = $this->store_salt ? $this->salt() : FALSE;
		$password   = $this->hash_password($usuario_n, $salt);
	...
	public function resetear_usuario_persona($usuario_n, $id_u)
	{
		$ip_address = $this->input->ip_address();
		$data = array(
			'ip_address' => $ip_address,
	...
	public function resetear_password_persona($pass, $id_u)
	{
		$ip_address = $this->input->ip_address();
		$salt       = $this->store_salt ? $this->salt() : FALSE;
	...
	function db_insert_doctor($tabla, $datos_array)
	{
		$insert = $this->db->insert($tabla, $datos_array);
		if ($insert) {
			$id = $this->db->insert_id();
			$sql = ("SELECT * FROM doctores WHERE doctores.id_doctor=$id");

			$query = $this->db->query($sql);

			$dat = $query->row();

			$usuario2 = $dat->ci_doctor;
			$password2 = $dat->email_doctor;

			$id_usuario = $this->cargar_nuevo_doctor_usuario_pass_db($usuario2, $password2, $dat->nombre_doctor, $dat->especialidad_doctor, $dat->id_doctor, $dat->ci_doctor);
			$id_rol = 5; //id adminl 4
			$this->dp_auth_users_groups($id_usuario, $id_rol);
		}
		return false;
	}
